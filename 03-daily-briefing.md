...
4. **An LLM endpoint** for the final selection pass only. (Same reasoning-model caveat as
   Build Spec 02 §4 applies — a standard chat model is safest for structured selection.)
...
- Reuse the LLM pitfalls from Build Spec 02 §4 (reasoning-model emptiness, proxy-on-loopback,
...
rules implemented (priority weights, recency, dedup, caps, floors); which LLM endpoint does
the final pick and whether it's reasoning-safe; the delivery target + schedule entry; and a
